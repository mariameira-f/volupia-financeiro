...
function applyPermissions(){
  ...
  tabs.forEach(t=>{
    ...
    if(p==="ex-aluna"&&["presidencia","joias","caixinha","auditoria","admin"].includes(tab))t.classList.add("disabled");
...
async function loadResumoPresidencia(pid){
  ...
  const rows=Object.entries(totais).filter(([_,v])=>Math.abs(v)>0.01).map(([uid,val])=>{
    ...
    return`<tr>
      ...
      <td class="val-hide" style="font-weight:700;color:${val>0?"var(--vermelho)":"var(--verde)"}">${val>0?fmtBRL(val):"-"+fmtBRL(Math.abs(val))}</td>
      <td>—</td>
      <td>—</td>
    </tr>`;
  });
  if(rows.length)tbody.innerHTML=rows.join("");
  else tbody.innerHTML='<tr><td colspan="5" style="text-align:center;color:#a8a29e;padding:20px">Nenhum valor a dividir</td></tr>';
}
...
async function loadJoias(){
  ...
  if(!data||!data.length){tbody.innerHTML='<tr><td colspan="16" style="text-align:center;color:#a8a29e;padding:30px">Nenhuma joia registrada</td></tr>';return;}
  tbody.innerHTML=data.map(j=>{
    const p=j.profiles||{};
    // Group payments by month
    const pagByMes={};
    (j.joias_pagamentos||[]).forEach(pg=>{
      if(!pagByMes[pg.mes])pagByMes[pg.mes]=[];
      pagByMes[pg.mes].push(pg);
    });
    let totalPago=0;
    let cells="";
    for(let m=1;m<=12;m++){
      const pgs=pagByMes[m]||[];
      if(!pgs.length){cells+=`<td style="text-align:center;color:#d6d3d1;font-size:11px">—</td>`;continue;}
      const pagos=pgs.filter(p=>p.pago);
      const pendentes=pgs.filter(p=>!p.pago);
      const valPago=pagos.reduce((s,p)=>s+parseFloat(p.valor),0);
      const valPend=pendentes.reduce((s,p)=>s+parseFloat(p.valor),0);
      totalPago+=valPago;
      if(pendentes.length&&!pagos.length){
        // All pending - clickable
        cells+=`<td style="background:#fee2e2;text-align:center;font-size:11px;cursor:pointer" onclick="pagarJoia(${pendentes[0].id})">${fmtBRL(valPend)}</td>`;
      }else if(pagos.length&&!pendentes.length){
        // All paid
        cells+=`<td style="background:#d1fae5;text-align:center;font-size:11px;font-weight:700;color:#065f46">${fmtBRL(valPago)}</td>`;
      }else{
        // Mix
        cells+=`<td style="background:#fef3c7;text-align:center;font-size:10px;cursor:pointer" onclick="pagarJoia(${pendentes[0].id})"><span style="color:#065f46">${fmtBRL(valPago)}</span><br><span style="color:#991b1b">${fmtBRL(valPend)}</span></td>`;
      }
    }
    const falta=parseFloat(j.valor_integral)-totalPago;
    return`<tr><td>${j.ano}</td><td><strong>${p.apelido||p.nome||"—"}</strong></td><td class="val-hide" style="font-weight:700">${fmtBRL(j.valor_integral)}</td>${cells}<td class="val-hide" style="font-weight:700;color:${falta>0.01?"#dc2626":"#059669"}">${falta>0.01?fmtBRL(falta):"✅ OK"}</td></tr>`;
  }).join("");
}

async function pagarJoia(pgId){
  // Get this parcela and its joia info
  const{data:pg}=await sb.from("joias_pagamentos").select("*,joias(id,valor_integral,moradora_id)").eq("id",pgId).single();
  if(!pg)return;
  const joiaId=pg.joias.id;
  // Get all unpaid parcelas for this joia
  const{data:todas}=await sb.from("joias_pagamentos").select("*").eq("joia_id",joiaId).eq("pago",false).order("mes");
  const totalPendente=todas.reduce((s,p)=>s+parseFloat(p.valor),0);

  const tipo=prompt("Tipo de pagamento:\n1 = Dinheiro\n2 = Produto doado para a casa\n\nDigite 1 ou 2:","1");
  if(!tipo)return;
  let obs="";
  if(tipo==="2"){
    obs=prompt("Descreva o produto doado:");
    if(!obs)return;
  }
  const valorStr=prompt(`Valor do pagamento?\n\nParcela atual: ${fmtBRL(pg.valor)}\nTotal pendente: ${fmtBRL(totalPendente)}\n\n(O valor pode ser maior que uma parcela — parcelas serão quitadas na ordem)`,pg.valor);
  if(!valorStr)return;
  const valorPago=parseFloat(valorStr);
  if(isNaN(valorPago)||valorPago<=0){alert("Valor inválido.");return;}
  ...
  // Mark parcelas as paid until value is consumed
  let restante=valorPago;
  let parcelasPagas=0;
  for(const parcela of todas){
    if(restante<=0)break;
    const valParcela=parseFloat(parcela.valor);
    if(restante>=valParcela-0.01){
      // Pay full parcela
      await sb.from("joias_pagamentos").update({pago:true,valor:valParcela,data_pagamento:dataISO,marcado_por:currentProfile.id}).eq("id",parcela.id);
      restante-=valParcela;
      parcelasPagas++;
    }else{
      // Partial: pay what we can and reduce the parcela value
      await sb.from("joias_pagamentos").update({pago:true,valor:restante,data_pagamento:dataISO,marcado_por:currentProfile.id}).eq("id",parcela.id);
      // Create new parcela for the remaining amount
      const sobra=valParcela-restante;
      await sb.from("joias_pagamentos").insert({joia_id:joiaId,mes:parcela.mes,valor:sobra,pago:false});
      restante=0;
      parcelasPagas++;
    }
  }

  const tipoLabel=tipo==="2"?"produto":"dinheiro";
  await audit("Pagar joia","joias_pagamentos",pgId,{tipo:tipoLabel,valor:valorPago,parcelas_quitadas:parcelasPagas,obs});
  toast(`${fmtBRL(valorPago)} pago (${parcelasPagas} parcela${parcelasPagas>1?"s":""})${obs?" — "+obs:""}`);
  loadJoias();
}
...
async function novaJoia(){
  ...
  abrirModal("Registrar Joia",`
    <div style="display:grid;gap:10px">
      ...
      <div><label>Ano Início</label><input type="number" id="mJoiaAno" value="${new Date().getFullYear()}"></div>
      <div><label>Valor Integral (½ salário mínimo)</label><input type="number" id="mJoiaValor" step="0.01" value="${metade}"></div>
      <div><label>Nº de Parcelas</label><input type="number" id="mJoiaNParcelas" min="1" max="48" value="12"></div>
      <p style="font-size:10px;color:#78716c;margin:0">A joia pode ser paga ao longo de vários anos. Parcelas serão distribuídas mês a mês a partir de janeiro.</p>
...
async function salvarJoia(){
  ...
  const nParcelas=parseInt(document.getElementById("mJoiaNParcelas").value)||12;
  const parcVal=+(valor/nParcelas).toFixed(2);
  const{data:joia,error}=await sb.from("joias").insert({moradora_id:morId,ano,valor_integral:valor,data_transicao:new Date().toISOString().split("T")[0]}).select().single();
  if(error){alert("Erro: "+error.message);return;}
  // Criar parcelas (max 12 per year, overflow to next years)
  const parcelas=[];
  for(let i=0;i<nParcelas;i++){
    const mes=(i%12)+1;
    parcelas.push({joia_id:joia.id,mes:mes,valor:parcVal});
  }
  await sb.from("joias_pagamentos").insert(parcelas);
  await audit("Registrar joia","joias",joia.id,{moradora:morId,valor,ano,parcelas:nParcelas});
  fecharModal();toast(`Joia registrada! ${nParcelas} parcelas de ${fmtBRL(parcVal)}`);loadJoias();
}
...
function exportarPedidosPDF(){
  const rows=document.querySelectorAll("#camTodosTbody tr");
  if(!rows.length){alert("Sem pedidos.");return;}
  let h=`<html><head><meta charset="utf-8"><title>Pedidos Camisas</title><style>body{font-family:Arial,sans-serif;font-size:11px;padding:20px}h1{font-size:16px;color:#62162f}table{width:100%;border-collapse:collapse;margin-top:10px}th,td{border:1px solid #ccc;padding:5px 7px;text-align:left}th{background:#62162f;color:#fff;font-size:9px;text-transform:uppercase}tr:last-child td{font-weight:bold;background:#f5f5f4}.extra{background:#fffbeb}</style></head><body>`;
  h+=`<h1>Volúpia — Pedidos de Camisas ${camAnoAtual}</h1><p>${new Date().toLocaleDateString("pt-BR")} ${new Date().toLocaleTimeString("pt-BR")}</p>`;
  h+=`<table><thead><tr><th>Pessoa</th><th>Perfil</th><th>Detalhes</th><th>Qtd</th><th>Valor</th><th>Pago</th><th>Falta</th><th>Status</th></tr></thead><tbody>`;
  rows.forEach(r=>{
    const tds=r.querySelectorAll("td");
    if(tds.length>=8){
      const isExtra=r.style.background?.includes("fffbeb");
      const isTotal=r.style.background?.includes("fafaf9");
      h+=`<tr${isExtra?' class="extra"':''}${isTotal?' style="font-weight:bold;background:#f5f5f4"':''}>`;
      for(let i=0;i<Math.min(8,tds.length);i++)h+=`<td>${tds[i].innerText||tds[i].textContent}</td>`;
      h+=`</tr>`;
    }
  });
